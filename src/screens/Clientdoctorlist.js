...
const ClientDoctorList = ({ navigation, header = true }) => {
  ...
  const resolveCityName = useCallback((doctor) => {
    // إذا كان الاسم موجوداً مباشرةً نعرضه، وإلا نحلّه من الـ id
    return doctor.city || doctor.city_name || getCityNameById(doctor.city_id);
  }, [getCityNameById]);

  const resolveAreaName = useCallback((doctor) => {
    return doctor.area || doctor.area_name || getAreaNameById(doctor.area_id);
  }, [getAreaNameById]);
  ...
  const fetchFilteredDoctors = useCallback(
    ...
    [user?.id, token, filters.searchTerm, filters.city_id, filters.area_id, filters.speciality_id]
  );
    ...
    async (currentPage = 1, isRefresh = false) => {
      ...
      try {
        ...
        const response = await get(
          `doctors?page=${currentPage}&user_id=${user?.id}&limit=10`,
          { headers }
        );
