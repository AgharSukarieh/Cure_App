		...
		<SafeAreaView style={styles.container}>
			...
			<View style={{ marginVertical: 30, width: "90%", alignSelf: "center" }}>
				<View style={{ flexDirection: "row", justifyContent: "space-between" }}>
					...
					{moment(date, "YYYY-M-D").format("YYYY-MM-DD") == moment().format("YYYY-MM-DD") ?
			...
			<View style={style.container}>
				...
				<TableView
					...
					onEndReached={false}
					renderItem={({ item }) => <DailyMedicalItemTable refresh={(e) => mainrow()} item={item} />}
				/>
			</View>

			<DailyaddModel show={modal} hide={() => {
				setModal(false);
			}} submit={(e) => {
				mainrow(e);
			}} date={date} area={area} />
			<Sweetalert show={alert} hide={() => {
				setalert(false);
