...
const Logo = ({ iconSize = 161 }) => {
  ...
  const fillOpacity = useRef(new Animated.Value(0)).current; // استخدام قيمة واحدة للتعبئة

  useEffect(() => {
    // إخفاء الخطوط بعد الرسم
    const strokeAnimation = Animated.timing(strokeDashoffset, {
      toValue: 0,
      duration: 2000, // مدة الرسم
      useNativeDriver: false,
    });

    // إظهار التعبئة بعد انتهاء الرسم
    const fillAnimation = Animated.timing(fillOpacity, {
      toValue: 1,
      duration: 1000, // مدة ظهور التعبئة
      useNativeDriver: false,
    });

    // بدء الحركتين بالتسلسل
    Animated.sequence([
      strokeAnimation,
      fillAnimation
    ]).start();
      ...
      >
        <G transform="translate(0.000000,140.000000) scale(0.100000,-0.100000)">
          ...
          <AnimatedPath
            ...
            stroke="#183E9F"
            strokeWidth="6"
            strokeDasharray="3000" // استخدام نفس القيمة لجميع المسارات
            strokeDashoffset={strokeDashoffset}
            strokeLinecap="round"
            strokeLinejoin="round"
          />
...
const styles = StyleSheet.create({
  container: {
    ...
    alignItems: 'center',
    backgroundColor: 'transparent', // خلفية شفافة لسهولة الدمج
  },
});
